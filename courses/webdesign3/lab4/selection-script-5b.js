...
function init() {   // Initialize global variables and link functions to buttons
	...
	addListener(document.getElementById("goBtn"), "click", function() { location.href = "slideshow-5b.htm"; });
...
var category = {
	...
	// Local variables
    menu: null,														// Refer to the category menu element
	...
	request: function(nr) {   // Make an AJAX-call to read chosen file
		...
		// Run AJAX
		request.open("GET", "categories/images" + nr + ".json", true);
		...
		request.onreadystatechange = function () {					// Function to check status in the communication
			if (request.readyState == 4 && request.status == 200)	// When communication is ready readyState is 4 and if file exists status is 200 (OK)
				category.get(JSON.parse(request.responseText));		// Interpret data from read file
		};
		
	}, // End request
	
	get: function(json) {   // Function to interpret JSON-code and to create HTML-cod with content shown on page
		
		// Local variables
		var i;																			// Loop counter
		var HTMLcode = "<h3>" + json.category + "</h3>";								// Textstring for the HTML-code to be added
		...
		for (i=0; i<json.image.length; i++)							// Going through image and add each read image into the temporary HTML code
			HTMLcode += '<div><p>' + json.image[i].caption + '</p><img src="' + json.image[i].url + '" alt=""></div>';
...
var ssStyle = {
	...
	save: function() {   // Save each chosen style into a cookie when pressing the "Spara stil" button
